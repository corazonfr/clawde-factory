    ...
    loadAudioFile: function (src) {
        ...
        if (AudioEngine.AUDIO_BUFFER_CACHE[src]) {
            ...
            if (window.location.protocol === 'file:') {
                // fetch() is not permitted on file://, and _getMissingAudioBuffer
                // deliberately beeps. Hand back real silence and cache it.
                if (!AudioEngine._audioContext) { AudioEngine._initAudioContext(); }
                var ctx = AudioEngine._audioContext;
                var silence = ctx.createBuffer(1, 1, ctx.sampleRate);
                AudioEngine.AUDIO_BUFFER_CACHE[src] = silence;
